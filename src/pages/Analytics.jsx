...
const Analytics = () => {
  ...
  const monthlyComparison = useMemo(() => {
    ...
    const lastMonthExpenses = yearTransactions.filter(t => 
      t.type === 'expense' && 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20 sm:pb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
            ...
            <p className={`text-2xl font-bold ${monthlyComparison.isHigher ? 'text-red-600' : 'text-green-600'}`}>
              {monthlyComparison.isHigher ? '+' : '-'}{Math.abs(monthlyComparison.percentage)}%
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 bg-gradient-to-r from-primary to-secondary rounded-lg p-6 text-white">
          ...
          <div className="space-y-2">
            ...
            {anomalies.length > 0 && (
              <p>• You had {anomalies.length} unusual expenses recently. Consider if these were necessary purchases.</p>
            )}
          </div>
        </div>
      </div>
